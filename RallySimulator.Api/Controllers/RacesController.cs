...
namespace RallySimulator.Api.Controllers
{
    ...
    public sealed class RacesController : ApiController
    {
        ...
        [HttpGet(ApiRoutes.Races.GetRaceById)]
        [ProducesResponseType(typeof(RaceResponse), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> GetRaceById(int raceId) =>
            await Maybe<GetRaceByIdQuery>.From(new GetRaceByIdQuery(raceId))
                .Bind(command => Sender.Send(command))
                .Match(Ok, NotFound);
